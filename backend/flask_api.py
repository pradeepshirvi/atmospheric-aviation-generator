...
def get_presets():
    presets = {
        'weather_conditions': {
            'clear_sky': {'surface_temp': 20, 'surface_pressure': 1013.25, 'surface_humidity': 40},
    ...
    try:
        ...
        if not MATPLOTLIB_AVAILABLE:
            ...
            return jsonify({
                ...
                'metadata': {'type': image_type, 'generated_at': datetime.now().isoformat()}
            })
    ...
    try:
        if psutil:
            ...
            return jsonify({
                ...
                'service': {'status': 'healthy', 'active_models': ['GAN-v1', 'VAE-TimeLength-100']},
                'timestamp': datetime.now().isoformat()
            })
        else:
        ...
        return jsonify({
            ...
            'service': {'status': 'healthy (fallback)', 'active_models': ['GAN-v1']}
        })
...
def evaluate_training():
    try:
        metrics = {
            'real_model': {'mae': 1.25, 'rmse': 1.85, 'r2': 0.94},
            'synthetic_model': {'mae': 1.65, 'rmse': 2.35, 'r2': 0.91}
        }
        viz_data = []
        for i in range(50):
            alt = i * 400
